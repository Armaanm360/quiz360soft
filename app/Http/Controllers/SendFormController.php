    ...
    public function formProcessEntry(Request $request)
    {
        ...
        if ($request->process_type == 'forward') {
            ...
            if (Auth::user()->user_type == 'ENTRY_USER') {
                $all_user = User::where('status', 1)->where('user_category', $same)->where('user_type', 'AUTHORIZE_USER')->get();
            } else {
                $all_user = User::where('status', 1)->where('user_category', $same)->where('user_type', 'ENTRY_USER')->get();
            }
