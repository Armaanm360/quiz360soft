...
class UserForm extends Model
{
	...
	public function getDivisionNameCollege($id)
	{
		$var = DB::table('college_division')->where('college_div_id', $id)->get();

		if (isset($var[0])) {
			return $var[0]->college_division;
		}
	}
	public function subjectName($id)
	{
		$var = DB::table('college_subject')->where('college_sub_id', $id)->get();

		if (isset($var[0])) {
			return $var[0]->college_subject_name;
		}
	}

	public function getUserType($authid)
	{
		$table = DB::table('user_type')->where('sl_user_id',Auth::user()->id)->get();
		if (isset($table[0])) {
			return $table[0]->get_user_type;
		}
	}
	...
	public function subjectNameSpecial($id)
	{
		$var = DB::table('college_subject')->where('college_sub_id', $id)->get();

		if (isset($var[0])) {
			$str = $var[0]->college_subject_name;
			$lower = strtolower($str);
			$subst = substr($lower, 0, 3);
			return str_replace(' ', '_', $subst);
		}
	}

	public function quizCreator($creator_id)
	{
		$var = DB::table('users')->where('id', $creator_id)->get();
		if (isset($var[0])) {
			return $var[0]->name;
		}
	}
  ...
  public function getTopics($subject_id)
	{
	 return DB::table('subjective_quiz_table')->where('subjective_sub_id',$subject_id)->count();

	}
  public function getTotalQuestion($subject_id)
	{
	return	$data['total_questions'] = DB::table('subjective_quiz_table')->select('question_table')->where('subjective_sub_id', $subject_id)->sum('quiz_number');
