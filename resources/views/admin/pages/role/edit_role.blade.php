                                    ...
                                    <div class="row">
                                        <div class="form-group col-6">
                                            ...
                                            <input type="text" class="form-control" name="name" id="name"
                                                value="{{ $roleData['name'] }}" placeholder="Enter Name">
                                        ...
                                        <div class="form-group col-6">
                                            ...
                                            <select class="form-control select2" id="institution_id" name="institution_id"
                                                ...
                                                @foreach ($all_institutions as $row)
                                                    <option value="{{ $row->id }}"
                                                        @if ($row->id == $roleData->institution_id) selected @endif>
                                                        {{ $row->name }}</option>
                                                @endforeach
                                            </select>
                                                ...
                                                @foreach ($val as $row)
                                                    <div class="selectgroup selectgroup-pills">
                                                        <label class="selectgroup-item">
                                                            <input type="checkbox"
                                                                id="inlineCheckbox{{ $key }}_{{ $loop->iteration }}"
                                                                name="permissions[]" value="{{ $row->id }}"
                                                                class="selectgroup-input"
                                                                @if (in_array($row->id, $role_wise_permission_array)) checked="" @endif>
                                                            <span class="selectgroup-button"
                                                                for="inlineCheckbox{{ $key }}_{{ $loop->iteration }}">
                                                                {{ $row->name }} </span>
                                                        </label>
                                                    </div>
                                                @endforeach
